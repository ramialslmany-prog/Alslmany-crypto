...
npm run backtest -- --top 30 --timeframe 1h --years 1      # أكثر 30 عملة سيولة
```

`--top` يأخذ الترتيب من قاعدتك أنت — متوسّط القيمة المتداولة في كل شمعة داخل
النافذة نفسها — لا من طلب مباشر للمنصّة، لأن اختباراً يجلب بياناته أثناء تشغيله
تتغيّر نتيجته بتغيّر يوم تشغيله. تُستبعد أزواج العملات المستقرة والرموز ذات
الرافعة **قبل** القطع، وتُستبعد عملة لا تملك نصف النافذة على الأقل (التاريخ
القصير يضخّم المتوسّط)، والبيتكوين تُضاف دائماً دون أن تستهلك أحد المقاعد.
...
```bash
npm run build      # مرة واحدة بعد كل تحديث
npm start          # البوت والموقع معاً
```

ثم افتح **http://localhost:3000**. `Ctrl+C` يوقف الاثنين.

البوت يكتب قاعدة البيانات والموقع يقرأها؛ لا فائدة لأحدهما وحده — ولذلك إن
توقّف أحدهما يُوقَف الآخر. موقع يقرأ قاعدة لا يحدّثها أحد يبدو سليماً وهو
متأخّر ساعات، والتوقّف الظاهر أصدق من ذلك.

```bash
npm start -- --timeframe 4h --port 3100   # إطار أو منفذ آخر
```
...
npm run dev                      # الموقع في وضع التطوير
npm run seed && npm run dev:demo # بيانات وهمية لرؤية الصفحات قبل أي تشغيل
```
